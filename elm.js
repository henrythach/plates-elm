...
Elm.Main.make = function (_elm) {
   ...
   var update = F2(function (action,model) {
      ...
      if (_p0.ctor === "Increment") {
            ...
            var _p1 = _p0._0;
            return _U.cmp(model - _p1,45) < 0 ? 45 : model - _p1;
         }
   });
   var Decrement = function (a) {    return {ctor: "Decrement",_0: a};};
   var Increment = function (a) {    return {ctor: "Increment",_0: a};};
   var calculate$ = F2(function (weight,stack) {
      calculate$: while (true) if (_U.eq(weight,0)) return stack; else {
            var _v1 = weight - 5,_v2 = A2($Basics._op["++"],stack,_U.list([5]));
            weight = _v1;
            stack = _v2;
            continue calculate$;
         }
   });
   var calculate = function (weight) {
      return _U.cmp(weight,45) < 0 || !_U.eq(A2($Basics._op["%"],weight,5),0) ? _U.list([]) : A2(calculate$,weight,_U.list([]));
   };
   var stackOfWeights = function (weight) {
      return A2($Html.ul,
      _U.list([]),
      A2($List.map,function (n) {    return A2($Html.li,_U.list([]),_U.list([$Html.text($Basics.toString(n))]));},calculate(weight)));
   };
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.h1,
              _U.list([]),
              _U.list([A2($Html.span,_U.list([$Html$Attributes.id("targetWeightSpan")]),_U.list([$Html.text($Basics.toString(model))])),$Html.text("lbs")]))
              ,A2($Html.button,_U.list([$Html$Attributes.$class("stepper"),A2($Html$Events.onClick,address,Decrement(90))]),_U.list([$Html.text("-90")]))
              ,A2($Html.button,_U.list([$Html$Attributes.$class("stepper"),A2($Html$Events.onClick,address,Decrement(5))]),_U.list([$Html.text("-5")]))
              ,A2($Html.button,_U.list([$Html$Attributes.$class("stepper"),A2($Html$Events.onClick,address,Increment(5))]),_U.list([$Html.text("+5")]))
              ,A2($Html.button,_U.list([$Html$Attributes.$class("stepper"),A2($Html$Events.onClick,address,Increment(90))]),_U.list([$Html.text("+90")]))
              ,stackOfWeights(model)]));
   });
   var plates = _U.list([45,35,25,10,5,2.5]);
   var model = 45;
